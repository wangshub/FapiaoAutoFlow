...
python -m fapiao run            # 默认读 config.yaml + .env
...
两层配置,**密钥只进 `.env`(已 gitignore),绝不写入代码或 config.yaml**:

- `.env` — `IMAP_HOST/PORT/USER/PASSWORD`(163 用授权码,非登录密码)、`LLM_API_KEY/BASE_URL/MODEL/VISION_MODEL/TEMPERATURE/MAX_TOKENS`。模板见 `.env.example`。
- `config.yaml` — 非敏感项(收件夹、主题关键词、min_confidence、超时、归档路径)。模板见 `config.example.yaml`。

AI 用的是 **OpenAI 兼容接口**(`openai` 包,非 dashscope 原生 SDK),所以换任何兼容厂商只改 `.env` 的 `LLM_BASE_URL/MODEL`。`config.py` 里 `vision_model` 留空则复用 `llm_model`。
